...
const CMSValidatorSchema = Joi.object({
  title: Joi.string()
    .required()
    .label("title")
    .error(new Error("Please enter proper title!")),
  ...
  isExternalPageURL: Joi.boolean()
    .required()
    .label("isExternalPageURL")
    .error(new Error("Please enter correct isExternalPageURL!")),
  id: Joi.any().error(
    new Error(CustomErrorHandler.unprocessedEntity("Invalid cms ID!"))
  ),
});
...
const CMSStatusValidatorSchema = Joi.object({
  ...
  id: Joi.any()
    .required()
    .error(new Error(CustomErrorHandler.unprocessedEntity("Invalid cms ID!"))),
});
