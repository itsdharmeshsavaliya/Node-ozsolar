...
}).fields([{ name: 'banner_image', maxCount: 1 }, { name: 'banner_mobile_image', maxCount: 1 }]);
...
const bannercontroller = {
    async banner(req,res,next) {
        try {
            handleMultipartData(req,res, async (err)=>{
                ...
                if(!req.files) return next(CustomErrorHandler.servererror("Please select banner image!"));
                // if(!req.files.banner_mobile_image) return next(CustomErrorHandler.servererror("Please select banner mobile image!"));

                try {
                    const { error } = BannerValidator.BannerValidatorSchema.validate(req.body);
                    if(error) {
                        if(req.files.banner_image) await commonHelper.deleteFile(`${req.files.banner_image[0].path}`);
                        if(req.files.banner_mobile_image) await commonHelper.deleteFile(`${req.files.banner_mobile_image[0].path}`);
                        return next(error);
                    }
                    let old_sch;
                    let document;
                    ...
                    let { sortOrder, menuName, bannerType, title, slugname, click_url, description, isActive } = req.body;
    
                    if(req.body.id) {
                        const id = new ObjectId(req.body.id);
                        old_sch = await Banner.findById(id);
                        oldbannerimage = `${old_sch._doc.banner_image}`;
                        if(old_sch.banner_mobile_image) oldbanner_mobile_image = `${old_sch._doc.banner_mobile_image}`;
                        console.log(oldbannerimage);
                        console.log(oldbanner_mobile_image);
                    }
                    if(!old_sch) {
                        try{
                        document = await Banner.create({
                            menuName,
                            bannerType,
                            title,
                            ...
                            ...(req.files.banner_mobile_image && {banner_mobile_image: commonHelper.convertFilePathSlashes(req.files.banner_mobile_image[0].path)}),
                            isActive
                        });
                        return res.status(200).json({ status: 1, message: "banner created successfully" });
                        } catch (err) {
                            return next(err)
                        }
                    }else {
                        try{
                            const updateSet = {
                                ...
                                description,
                                // banner_image: commonHelper.convertFilePathSlashes(req.files.banner_image[0].path),
                                ...(req.files.banner_image && {banner_image: commonHelper.convertFilePathSlashes(req.files.banner_image[0].path)}),
                                ...(req.files.banner_mobile_image && {banner_mobile_image: commonHelper.convertFilePathSlashes(req.files.banner_mobile_image[0].path)}),
                                isActive
                            }
                            ...
                            if(document.banner_mobile_image) {
                                if(oldbanner_mobile_image !== document.banner_mobile_image) await commonHelper.deleteFile(`${oldbanner_mobile_image}`);
                            }
                            return res.status(200).json({ status: 1, message: "banner updated successfully" });
                        }catch (err) {
                            return next(err)
                        }
                    }
    ...
    async bannerstatus(req,res,next) {
        try {
            const { error } = BannerValidator.BannerStatusValidatorSchema.validate(req.body);
            if(error) {
                return next(error);
            }
            let document;
            ...
            try {
                ...
                return res.status(200).json({ status: 1, message: "banner status updated successfully" });
            } catch (err) {
                return next(err);
            }
        } catch (err) {
    ...
    async remove(req,res,next) {
        try {
            ...
            let bannerId = req.params.id;
             ...
             if (document.banner_image) await commonHelper.deleteFile(document.banner_image);
             if (document.banner_mobile_image) await commonHelper.deleteFile(document.banner_mobile_image);
