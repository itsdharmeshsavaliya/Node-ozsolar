...
const packagecontroller = {
    ...
    async package(req,res,next) {
        try {
            const { error } = PackageValidator.PackageValidatorSchema.validate(req.body);
            if(error) {
                return next(error);
            }
            let old_sch;
            let document;
            let { packageName , title, sortOrder, kwText, savings_upto_amount, saving_upto_text, amount, amount_text, description, isActive } = req.body;
            if(req.body.id) {
            ...
            if(!old_sch) {
                ...
                return res.status(200).json({ status: 1, message: "package created successfully" });
                } catch (err) {
                    return next(err)
                }
            }else {
                ...
                const updateSet = {
                 ...
                 return res.status(200).json({ status: 1, message: "package updated successfully" });
            }catch (err) {
                return next(err)
            }
            }
    ...
    async packagestatus(req,res,next) {
        try {
         const { error } = PackageValidator.PackageStatusValidatorSchema.validate(req.body);
         ...
         try {
            ...
            return res.status(200).json({ status: 1, message: "package status updated successfully" });
         } catch(err) {
            return next(err);
         } 
        } catch (err) {
