...
const socialMediacontroller = {
    async contact(req,res,next) {
        try {
            const { error } = SocialMediaValidator.SocialMediaValidatorSchema.validate(req.body);
            if(error) {
                if(req.file) await commonHelper.deleteFile(`${req.file.path}`);
                return next(error);
            }
            let document;
            let { Facebook, LinkedIn, Twitter, YouTube, Instagram, Pintrest } = req.body;
            ...
            if(!old_sch) {
                ...
                return res.status(200).json({ status: 1, message: "socialMedia created successfully" });
                } catch (err) {
                    return next(err)
                }
            }else {
                try{
                    ...
                    return res.status(200).json({ status: 1, message: "socialMedia updated successfully" });
                }catch (err) {
                    return next(err)
                }
            }
    ...
    async remove(req,res,next) {
        try {
            const { error } = SocialMediaValidator.idValidatorSchema.validate(req.params);
            if(error) {
                return next(error);
            }
