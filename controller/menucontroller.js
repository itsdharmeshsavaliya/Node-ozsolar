...
const menucontroller = {
    async menu(req,res,next) {
        try {
            ...
            let old_sch;
            let document;
            let { parentId, menuName, menuType, cms, sortOrder, slugmenu, menu_URL_unique_key, category, showInHeader, showInFooter, isActive } = req.body;
            ...
            if(!old_sch) {
                ...
                return res.status(200).json({ status: 1, message: "menu created successfully" });
                } catch (err) {
                    return next(err)
                }
            }else {
                try{
                    ...
                    return res.status(200).json({ status: 1, message: "menu updated successfully" });
                }catch (err) {
                    return next(err)
                }
            }
    ...
    async menustatus(req,res,next) {
        try {
         const { error } = MenuValidator.MenuStatusValidatorSchema.validate(req.body);
         if(error) {
            return next(error);
         }
         let document;
         ...
         try {
            ...
            return res.status(200).json({ status: 1, message: "menu status updated successfully" });
         } catch(err) {
            return next(err);
         } 
        } catch (err) {
