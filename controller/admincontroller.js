...
const adminController = {
  async register(req, res, next) {
    try {
      const { error } = AdminValidator.AdminRegisterValidatorSchema.validate(
        req.body
      );
      if (error) {
        return next(error);
      ...
      var tt = await TokenAdminSchema.create({
        userId: user._id,
        token: refresh_token,
        expiresAt: new Date(),
      });
      ...
      return res
        .status(200)
        .json({ status: 1, info, accessToken, token, message });
    } catch (error) {
      return next(error);
    }
  },
  ...
  async login(req, res, next) {
    try {
      const { error } = AdminValidator.AdminLoginValidatorSchema.validate(
        req.body
      );
      if (error) {
        return next(error);
      ...
      if (!user) {
        return res
          .status(400)
          .json({ status: 0, error: "username is not Registered!" });
      }
      ...
      try {
        ...
        if (r_t === "al") {
          ...
          console.log({ b: user._id, a: role });
          refresh_token = await JwtService.sign(
            { _id: user._id, role },
            "refresh"
          );
          // user.token = refresh_token;
          console.log("New Generated", refresh_token);

          await TokenAdminSchema.create({
            userId: user._id,
            token: refresh_token,
            expiresAt: new Date(),
          });
      ...
      return res
        .status(200)
        .json({ status: 1, info, accessToken, token: refresh_token, message });
    } catch (err) {
      return next(err);
    }
  },
  ...
  async changepassword(req, res, next) {
    try {
      const { error } =
        AdminValidator.AdminChangePasswordValidatorSchema.validate(req.body);
      if (error) {
        return next(error);
      ...
      var new_data = await AdminSchema.findByIdAndUpdate(
        { _id: admin._id },
        { password: hashedPassword },
        { new: true }
      );
  ...
  async logout(req, res, next) {
    try {
      const { error } = AdminValidator.refreshTokenValidatorSchema.validate(
        req.body
      );
      if (error) {
        return next(error);
      ...
      if (del.acknowledged === true) {
        ...
        res
          .status(400)
          .json({ status: 0, error: "Error in Logging out Admin!" });
      }
    } catch (error) {
      return next(error);
    }
