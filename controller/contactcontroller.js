...
const contactcontroller = {
    async contact(req,res,next) {
        try {
            const { error } = ContactValidator.ContactValidatorSchema.validate(req.body);
            if(error) {
                if(req.file) await commonHelper.deleteFile(`${req.file.path}`);
                return next(error);
            }
            let document;
            let { company_name, office_contact_no, office_address, google_map } = req.body;
            ...
            if(!old_sch) {
                ...
                try{
                    ...
                    return res.status(200).json({ status: 1, message: "contact updated successfully" });
                }catch (err) {
                    return next(err)
                }
            }
